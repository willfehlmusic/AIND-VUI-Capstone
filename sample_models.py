...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for layer in range(recur_layers):
        ...
        bn_rnn = BatchNormalization(name='bn_rnn_%d' % layer)(simp_rnn)
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(bn_rnn)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
...
def final_model(input_dim=161,
    ...
    for layer in range(recur_layers):
        ...
        rnn = Bidirectional(GRU(units, return_sequences=True, 
                                implementation=2))(rnn_drp)
        # Add batch normalization to bidirectional rnn layer
        bnn = BatchNormalization()(rnn)
        #Add dropout to avoid overfitting
        rnn_drp = Dropout(0.3)(bnn)
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(rnn_drp)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
