...
model_3 = deep_rnn_model(input_dim=161,#161, # change to 13 if you would like to use MFCC features
                         units=200,
                         recur_layers=2) 
...
# 
# __Answer:__ It seens that none of the models are over fitting to the data as the validation loss is still lower than the training loss. However, as they are almost equal for certain models, those models may be underfitting.
# 
# Model 0: This model has the worst performance partly due to its small size and simple configuration. Therefore, the memory structure of this model is severly limited.
# 
# Model 1: By adding Batch Normalization and Time Distributed Densley Connected Layer, the performance is greatly improved.
# 
# Model 2: Using a 1D Convolutional Neural Network takes longer to train but the model size is reduced.
# 
# Model 3: A deep RNN does not perform as well as a 1D Convolution Network.
# 
# Model 4: A bidrectional RNN reduces the model size from Model 3 and gives simmilar performance. Therefor, it can be presumed that it can be nested to give better results.
# 
# 
# This seems to indicate that a bidrectional RNN and Convolutional Neural Network are both important. However, there is likely the need for many layers and perhaps using the full spectrum instead of the MFCCs. This will increase computation time severely.
...
# 
# __Answer:__ My model uses a 1D Convolution with Batch Normalization. This then feeds into a 3 Layer, Recurrent Bidrectional Neural Network of 200 node each. Each Bidirectional LSTM has a dropout rate of 30%. This then feeds to a TimeDistributed Dense Connection Layers.
# Adding more layers would be a better idea. But this seemed to give decent results.
